...
- **Every abort reaches the job through the tick.** The level-identity reset sits in `extract` [L101-106](src/client/java/dev/kelianmao/mobwalk/client/surface/CollisionSurfaceOverlay.java) while `advanceFlood()` runs on `ClientTickEvents.END_CLIENT_TICK`, and the job's `hopCount` + `WorldSurfaceIndex` are plain `HashMap`s that one thread may touch at a time. So step 3 confirms which thread extraction runs on ([rendering.md](docs/rendering.md) Threading places compute on the client/extraction thread) and routes the abort through the tick driver: either `advanceFlood()` tests level identity itself, or `extract` raises a `volatile` flag the driver consumes before stepping a ring. The three arm-new triggers (`onUseItem`, `reselectWithMobProfile`, `dumpFloodDebug`) and the two other clears (`onUseItem` at nothing, `clearSelectionForSoftDisable`) already run on the client thread, so latest-wins covers them directly.
