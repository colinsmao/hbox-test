...
- **One completion path, one driver.** `stepRing()` is the unit; reaching the empty queue triggers a single **`finish()`** — run the Phase-2 passes, build the new immutable lists, reference-swap `result`/`occluders`/`downSkirts`/`holes`/`hazards`, and `publish()`. `advanceRings(budgetNanos)` is the only way there: it calls `stepRing()` until the wall-time budget is spent, always finishing the ring in progress, and at an unlimited budget takes every remaining ring in one call — today's exact synchronous result. `SurfaceSelection` holds the active job across frames; `select(...)` is an **arm/seed** ("cancel any job, build a fresh one seeded from the origin wave", returns immediately — no expansion, no finalize) for every budget, including the unlimited one, which the next frame then finishes whole.
- **Drive it from the overlay.** `CollisionSurfaceOverlay.extract` calls a private `advanceFlood()` each frame (§2). `onUseItem`/`reselectWithMobProfile` arm a job instead of blocking; `publish()` happens inside `finish()`. `advanceFlood()` is also where the setting's `0`-means-unlimited sentinel is translated to `Long.MAX_VALUE`, since a `0` budget at the job level buys the minimum of one ring.
